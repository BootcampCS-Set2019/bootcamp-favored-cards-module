...
protocol FavoredCardsPresenterProtocol {
    func loadSetsAndTypes() -> Future<Void, APIError>
    func loadAllCardsOfNextSet() -> Future<CardSetHelper, FavoredCardsPresenterError>
}

enum FavoredCardsPresenterError: Error {
    case setsAndTypesNotLoaded
    case notExistMoreSets
    case cardsNotLoaded
}

class FavoredCardsPresenter: FavoredCardsPresenterProtocol {
    ...
    fileprivate func loadCard(with type: String, and set: CardSet) -> Future<CardTypeHelper, FavoredCardsPresenterError> {
